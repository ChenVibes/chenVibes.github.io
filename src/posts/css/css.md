...
1.display:flex.设置自己的显示方式为伸缩布局，可以让子元素进行伸缩排列。

2.flex-direction 调整主轴方向（默认为水平方向）

3. justify-content:设置子元素在主轴方向布局的对齐方式。

4.align-items:设置子元素在侧轴方向的对齐方式。

5.flex-wrap:设置当子元素的宽度大于父元素的宽度时，是否换行。

6.align-content:用来设置侧轴方向的布局方式。如果子元素只有一行，这个属性无效。

子元素相关的属性

7.给子元素设置 flex 属性，可以对父元素的宽度或高度进行分配。

8.align-self 属性设置子元素自己在侧轴方向的布局方式，优先级要高于 align-items

9.order 属性用于对子元素进行排序，数值越小越靠前。
